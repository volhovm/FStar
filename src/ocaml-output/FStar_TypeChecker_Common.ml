...
let (find_nearest_preceding_col_info :
  ...
  fun col  ->
    fun col_infos  ->
      let rec aux out uu___18_1182 =
        match uu___18_1182 with
        | [] -> out
        | (c,i)::rest ->
            if c > col
            then out
  ...
  fun ty_map  ->
    fun db  ->
      fun info  ->
        ...
        let info1 =
          let uu___21_1353 = info  in
          let uu____1354 = ty_map info.identifier_ty  in
          {
            identifier = (uu___21_1353.identifier);
    ...
    fun id1  ->
      fun ty  ->
        fun range  ->
          ...
          let uu___22_1544 = table  in
          {
            id_info_enabled = (uu___22_1544.id_info_enabled);
            id_info_db = (uu___22_1544.id_info_db);
...
let (id_info_toggle : id_info_table -> Prims.bool -> id_info_table) =
  fun table  ->
    fun enabled  ->
      let uu___23_1598 = table  in
      let uu____1599 = enabled && (FStar_Options.ide ())  in
      {
        id_info_enabled = uu____1599;
        id_info_db = (uu___23_1598.id_info_db);
        id_info_buffer = (uu___23_1598.id_info_buffer)
      }
