...
let (extend : env -> Prims.string -> env) =
  fun env  ->
    fun x  ->
      let uu___27_4356 = env  in
      {
        names = ({ pretty = x } :: (env.names));
        names_t = (uu___27_4356.names_t);
        module_name = (uu___27_4356.module_name)
      }
  
let (extend_t : env -> Prims.string -> env) =
  fun env  ->
    fun x  ->
      let uu___28_4370 = env  in
      {
        names = (uu___28_4370.names);
        names_t = (x :: (env.names_t));
        module_name = (uu___28_4370.module_name)
      }
              ...
              (let env1 =
               ...
               let env2 =
                 FStar_List.fold_left
                   ...
                   tvars
                  in
               let rec find_return_type eff i uu___25_5179 =
                 match uu___25_5179 with
