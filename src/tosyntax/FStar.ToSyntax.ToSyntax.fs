...
let rec desugar_effect env d (quals: qualifiers) eff_name eff_binders eff_typ eff_decls attrs =
    ...
    let mandatory_members =
      let rr_members = ["repr" ; "return" ; "bind"; "wp_type"; "interp"; "mrelation"] in
    ...
    let actions_docs = actions |> List.map (fun d ->
        match d.d with
        | Tycon(_, _, [TyconAbbrev(name, action_params, _, defn), doc]) ->
    ...
    let lookup_or_dummy s =
        ...
        let t = match try_lookup_definition env l with
                | None -> mk Tm_unknown None Range.dummyRange
                | Some t -> Subst.close binders t
        in
        [], t
    in
    ...
    let se =
      ...
      { sigel =
        (Sig_new_effect({
           ...
           elaborated  = false;
           spec_dm4f   = false;
           interp      = lookup_or_none "interp";
           mrelation   = lookup_or_none "mrelation";
           actions     = actions;
