...
(* fill in type here *)
val encode: a:bytes{length a > 0 /\ length a < blocksize} -> bytes
let encode a = append a (pad (blocksize - length a))


(* fill in type here *)
val decode: block -> b:option bytes{b2t(Some? b) ==> b2t(let Some b' = b in length b' > 0 && length b' < blocksize)}
let decode b =
